...
class Model():
    ...
    def train_model(self, sc, model_name, num_iter, pid_training_set):
        ...
        for iteration in range(1, num_iter+1):
            if iteration == 1:
                linear_model = LinearRegressionWithSGD.train(rdd_training_labeled_data, intercept=True, iterations=100, step=1e-5)
            else:
                print linear_model.weights
                linear_model = LinearRegressionWithSGD.train(rdd_training_labeled_data,\
                        iterations=100, step=1e-5, intercept=True, initialWeights=linear_model.weights)
